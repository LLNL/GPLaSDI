...
def get_new_param(m_index, X_train, param_train, param_grid, x_grid, time_dim, space_dim, Dt, Dx):
    ...
    new_a_sys = str(new_a)
    new_w_sys = str(new_w)
    alefolderpath = "/Users/anderson316/Documents/gplasdi_github/GPLaSDI/BurgersEqn1D_randgrid/" # where runs are stored
    ale3dstring = "case_a" + new_a_sys+"_w"+new_w_sys #new folder name to hold run data
    ale3dnewrunfolder = alefolderpath + ale3dstring #path to new folder
    
    ##make new folder
    subprocess.run(["mkdir", ale3dstring],cwd = alefolderpath)
    subprocess.run(["cp", "paramrun_general.py", ale3dstring+'/'],cwd = alefolderpath) 
    subprocess.run(["cp", "input_file.sh", ale3dstring+'/'],cwd = alefolderpath)
    subprocess.run(["cp", "utils_solver.py", ale3dstring+'/'],cwd = alefolderpath) 
    

    #rename variablesfor solver file
    subprocess.run(["sed",'-i','', 's/aval/' + new_a_sys + '/g', "paramrun_general.py"],cwd = ale3dnewrunfolder) 
    subprocess.run(["sed",'-i','', 's/wval/'+ new_w_sys +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
    subprocess.run(["sed",'-i','', 's/time_dimval/'+ str(time_dim) +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
    subprocess.run(["sed",'-i','', 's/space_dimval/'+ str(space_dim) +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
    subprocess.run(["sed",'-i','', 's/x_minval/'+ str(np.min(x_grid)) +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
    subprocess.run(["sed",'-i','', 's/x_maxval/'+ str(np.max(x_grid)) +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
    subprocess.run(["sed",'-i','', 's/t_maxval/'+ str((time_dim-1)*Dt) +'/g', "paramrun_general.py"],cwd = ale3dnewrunfolder)
            
    #run ALE3D and wait for run to finish
    # subprocess.run(['sbatch -W input_file.sh'],shell=True,cwd = ale3dnewrunfolder)
    subprocess.run(['bash input_file.sh'],shell=True,cwd = ale3dnewrunfolder)
    
    pydatastring = '1DBurgers_a' + new_a_sys + '_w' + new_w_sys +'.npy'

    sys.path.append(ale3dnewrunfolder)

    new_X = np.load(ale3dnewrunfolder+'/'+pydatastring)
