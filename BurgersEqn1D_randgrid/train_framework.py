    ...
    def train(self):
        ...
        for iter in range(n_iter):
            ...
            if iter > 0 and iter < max_greedy_iter and iter % n_greedy == 0:
                ...
                coef_samples = [interpolate_coef_matrix(gp_dictionnary, param_grid[i, :], n_samples, n_coef, sindy_coef) for i in range(n_param)]
                Zis = [simulate_uncertain_sindy(gp_dictionnary, param_grid[i, 0], n_samples, Z0[i], t_grid, sindy_coef, n_coef, coef_samples[i]) for i in range(n_param)]

                m_index = get_max_std(autoencoder, Zis, n_param)
