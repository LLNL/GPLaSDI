...
class BayesianGLaSDI:
    ...
    def train(self):
        ...
        for iter in range(n_iter):
            ...
            loss_sindy, loss_coef, sindy_coef, sindy_coef_dt = find_sindy_coef(Z, Dt, n_train, time_dim, MSE)

            max_coef = np.abs(np.array(sindy_coef)).max()
            max_coef_dt = np.abs(np.array(sindy_coef_dt)).max()
            ...
            print("Iter: %05d/%d, Loss: %3.10f, Loss AE: %3.10f, Loss SI: %3.10f, Loss COEF: %3.10f, max|c|: %04.2f, max|c_dt|: %04.2f, "
                  % (iter + 1,
                     n_iter,
                     loss.item(),
                     loss_ae.item(),
            ...
            if iter > 0 and iter < max_greedy_iter and iter % n_greedy == 0:
                ...
                # m_index = get_sample_points(gp_dictionnary, param_grid, n_coef,sindy_coef,sample_points)
                
                # X_train, param_train = get_new_param_multiple(m_index,
                #                                      X_train,
                #                                      param_train,
                #                                      param_grid,
                #                                      x_grid,
        ...
        bglasdi_results = {'autoencoder_param': autoencoder.state_dict(), 'final_param_train': param_train,
                           'final_X_train': X_train, 'param_grid': param_grid,
                           'sindy_coef': sindy_coef, 'lr': self.lr, 'n_iter': n_iter,
                           'n_greedy': n_greedy, 'sindy_weight': sindy_weight, 'coef_weight': coef_weight,
                           'n_init': self.n_init, 'n_samples' : n_samples, 'n_a_grid' : n_a_grid, 'n_w_grid' : n_w_grid,
                           'a_grid' : self.a_grid, 'w_grid' : self.w_grid,
                           't_grid' : t_grid, 'x_grid' : x_grid, 'Dt' : Dt, 'Dx' : Dx,
