...
def solve_sindy(Z):
    ...
    for i in range(n_train):
        ...
        c_i = (np.linalg.lstsq(Zminus,Zplus)[0]).T
...
def simulate_sindy(sindy_coef, Z0, t_grid):
    ...
    for i in range(n_sindy):
        ...
        Z_i = np.zeros([len(t_grid),len(Z0[i])])
        Z_i[0,:] = Z0[i]
        for j in range(1,len(t_grid)):
            Z_i[j,:] = (c_i@Z_i[j-1,:].T).T
        # Z_i = Z_i.reshape(1, Z_i.shape[0], Z_i.shape[1])
        Z_i = np.atleast_3d(Z_i.T).T
...
def fit_gps(interpolation_data):
    ...
    for i in range(n_coef):
        ...
        # kernel = ConstantKernel() * Matern(length_scale_bounds = (10, 1e5), nu = 1.5)
        kernel = ConstantKernel() * RBF(length_scale_bounds = (.1, 1E5))
        # kernel = ConstantKernel() + ConstantKernel() * RBF(length_scale_bounds = (1, 1e5))

        # gp = GaussianProcessRegressor(kernel = kernel, n_restarts_optimizer = 10, random_state = 1)
        gp = GaussianProcessRegressor(kernel = kernel, n_restarts_optimizer = 10, random_state = 1,normalize_y=True)
        # gp = GaussianProcessRegressor(kernel = kernel, n_restarts_optimizer = 10, random_state = 1,normalize_y=True,alpha = 1E-8)
        gp.fit(X, y)
...
def find_sindy_coef(Z, Dt, n_train, time_dim, loss_function):
    ...
    dZdt, Zfordiff = compute_sindy_data(Z, Dt)
    ...
    for i in range(n_train):
        ...
        A = (torch.linalg.lstsq(Zminus,Zplus,driver='gels').solution).T #with new pytorch
       
        recon = [Zi[0,:]]

        for j in range(1,Zi.shape[0]):
           recon = recon + [torch.matmul(A, recon[-1])]

        recon = torch.stack(recon)


        loss_sindy += loss_function(Zi,recon)
        loss_coef += torch.linalg.norm(A)
        sindy_coef.append(A.detach().numpy())

        ##NOTE: The above calculates || Z - \hat{Z}||
        ##We can also calculate || \dot{Z} - \dot{\hat{Z}} || using the line below
        # Dop = (A - torch.eye(A.shape[0]))/Dt #FD approx of DMD time derivative
        # loss_sindy += loss_function(dZdt_i,(Dop@Z_i.T).T)
        # loss_coef += torch.linalg.norm(Dop)
        # sindy_coef.append(Dop.detach().numpy())


        


    return loss_sindy, loss_coef, sindy_coef, sindy_coef_dt
...
def get_sample_points(gp_dictionnary, param_grid, n_coef,sindy_coef,sample_points):
    ...
    X = (param_grid - np.min(param_grid,axis=0)) / (np.max(param_grid,axis=0) - np.min(param_grid,axis=0))
...
def get_new_param_multiple(m_index, X_train, param_train, param_grid, x_grid, time_dim, space_dim, Dt, Dx, X_test):
    ...
    for i in range(len(m_index)):
        new_a, new_w = param_grid[m_index[i], 0], param_grid[m_index[i], 1]
        
        new_X = torch.Tensor(np.atleast_3d(X_test[m_index[i],:,:].T).T)
