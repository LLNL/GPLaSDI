...
class BayesianGLaSDI:
    def __init__(self, autoencoder, model_parameters):
        ...
        self.poly_order = model_parameters['poly_order']
        self.sine_term = model_parameters['sine_term']
        ...
        self.n_proc = model_parameters['n_proc']
    ...
    def train(self):
        ...
        for iter in range(n_iter):
            ...
            loss_sindy, loss_coef, sindy_coef = find_sindy_coef(Z, Dt, n_train, MSE, poly_order, library_size, sine_term)

            max_coef = np.abs(np.array(sindy_coef)).max()

            loss_sindy /= n_train
            loss_coef /= n_train
            loss = loss_ae + sindy_weight * loss_sindy + coef_weight * loss_coef

            loss.backward()
            optimizer.step()


            #if keep_best_loss is True and loss.item() < best_loss and iter > n_iter - n_greedy:
            #if keep_best_loss is True and loss.item() < best_loss and iter > 0.95 * n_iter:		
            ...
            if iter > 0 and iter % n_greedy == 0:
                ...
                '''
                if keep_best_loss is True and iter > n_iter - n_greedy:
                ...
                '''
                Z0 = initial_condition_latent(param_grid, initial_condition, autoencoder)
