...
class FNOBlocks(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Adding features to store data from the latent dynamics evaluation!
        self.sindy_loss = None
        self.loss_coef = None
        self.coefs = None
	...
	# This can be adjusted regarding where u want the sindy operation to take place! For 3 layers, index = 1 means the central layer. WORKED THE BEST OUT OF ALL CASES!
        if index == 1:
            x_fno = self.convs[index](x, output_shape=output_shape, sindy_flag = True)
            
            # Update sindy loss terms!
            self.coefs = self.convs[index].coefs
            self.sindy_loss = self.convs[index].sindy_loss
            self.loss_coef = self.convs[index].loss_coefs
        else:
            x_fno = self.convs[index](x, output_shape=output_shape, sindy_flag = False)
        ...
        # Apply regularization layers for preventing overfitting in the model!
        # x = nn.Dropout(0.2)(x)
